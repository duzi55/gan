...
//
// 展开后卡片距离/位置的属性说明：
//   openX      -> --open-x      水平偏移（px），越大越靠右（负值靠左）
//   openY      -> --open-y      垂直偏移（px），相对锚点下移
//   openR      -> --open-r      展开时的旋转角度（deg）
//   openScale  -> --open-scale  展开时的缩放比例
//   整体上移量由 CSS 的 --cards-open-drop（负值 = 向上）控制，
//   最终垂直位置 = openY + --cards-open-drop
...
//hovering 方法 当图片展开被hovering时被hovering的图片置于顶层，并将open-x的值加100.并稍微放大卡片大小
// 层级（置顶）不在此处理，由 CSS 的 .portfolio-folder:hover .folder-card.is-hovered { z-index: 30 } 控制
const hovering = (card: FolderCard) => {
  return {
    // "--open-x": `${card.openX + 100}px`,
...
const cardStyle = (card: FolderCard): React.CSSProperties =>
  ({
    ...
    "--card-z": card.z, // 收拢时的层级交给 CSS 控制，展开/置顶也是CSS 控制
    width: card.w,
    height: card.h,
  }) as React.CSSProperties;
...
export default function PortfolioCarousel3D() {
  ...
  const handleCardLeave = () => {
    ...
    // 延迟 160ms 再清除，避免卡片移动后鼠标短暂脱离导致的抖动
  ...
  return (
    <div className="relative w-full h-[700px] sm:h-[840px] flex items-center justify-center select-none overflow-hidden">
      ...
      {/* Floating tags */}
      <div className="absolute top-[9%] left-[5%] sm:left-[12%] px-3 py-1.5 bg-white/80 backdrop-blur-sm rounded-full text-xs font-medium text-gray-600 shadow-sm animate-float-slow pointer-events-none">
        ui design
      </div>
      <div
        className="absolute top-[14%] right-[7%] sm:right-[15%] px-3 py-1.5 bg-lime-300/80 backdrop-blur-sm rounded-full text-xs font-semibold text-gray-800 shadow-sm animate-float-slow pointer-events-none"
        style={{ animationDelay: "1s" }}
      >
        youth
      </div>
      <div
        className="absolute bottom-[28%] left-[4%] sm:left-[10%] px-3 py-1.5 bg-white/80 backdrop-blur-sm rounded-full text-xs font-medium text-gray-600 shadow-sm animate-float-slow pointer-events-none"
        style={{ animationDelay: "2s" }}
      >
        comment
      </div>
      <div
        className="absolute top-[7%] right-[30%] sm:right-[32%] px-3 py-1.5 bg-pink-200/80 backdrop-blur-sm rounded-full text-xs font-medium text-gray-700 shadow-sm animate-float-slow pointer-events-none"
        style={{ animationDelay: "0.5s" }}
      >
        7/29
      </div>

      {/* Portfolio folder (responsive scale) */}
      <div
        className="relative z-10 scale-[0.58] sm:scale-[0.82] lg:scale-100"
          ...
          <div className="folder-shell absolute inset-0">
            ...
            <div className="folder-card-stack">
              {folderCards.map((card) => (
                <div
                  key={card.id}
                  className={`folder-card${hoveredId === card.id ? " is-hovered" : ""}`}
                  style={{
                    ...cardStyle(card),
                    ...(hoveredId === card.id ? hovering(card) : {}),
                  }}
            ...
            <div className="folder-front">
              ...
              <div className="absolute top-4 left-6 flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-black tracking-[0.18em] text-white/90 uppercase drop-shadow">
                  Portfolio
                </span>
              </div>
              ...
              <div className="absolute top-4 right-6">
                <div className="relative w-10 h-12">
                  <svg viewBox="0 0 48 56" className="w-full h-full" fill="none">
                    <path
                      d="M24 0C10.745 0 0 10.745 0 24c0 10.5 6.7 19.4 16 22.8V56l8-6 8 6v-9.2c9.3-3.4 16-12.3 16-22.8C48 10.745 37.255 0 24 0z"
                      fill="rgba(255,255,255,0.92)"
                      stroke="rgba(180,180,180,0.4)"
                      strokeWidth="1"
                    />
                    <text x="24" y="20" textAnchor="middle" fill="#64748b" fontSize="7" fontWeight="800" fontFamily="sans-serif">
                      GOOD
                    </text>
                    <text x="24" y="32" textAnchor="middle" fill="#64748b" fontSize="9" fontWeight="800" fontFamily="sans-serif">
                      IDEA
                    </text>
                  </svg>
                </div>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[11px] sm:text-xs text-gray-400 font-medium pointer-events-none whitespace-nowrap">
        Hover the folder to fan out cards
      </div>
    </div>
  );
}
