---
title: '七问七修：一个博客的迭代排障实录'
date: '2026-08-28'
excerpt: 从 CI 未触发到控制台报错——把上线路上遇到的每个问题按 现场→探讨→方案→避坑 记录在案，供日后排障对照引用
tags:
  - 架构
  - Web
...
<!-- 2026-08-31 Claude·去对话体（应用户反馈）：拆除「我问 AI、AI 作答」的问答叙事——
     标题、导语与正文改为面向读者的书面技术叙事，第一人称仅作作者视角，不再出现「你=AI」的对话对象 -->

墨境上线后的这几轮迭代，全部由报障与排障驱动。这篇文章**按问题组织**，每个问题还原四段——问题现场、探讨（当时怎么想的）、解决方案、避坑要点。写给未来的自己，也写给任何后来排障的人：问题描述是第一现场，别急着改代码，先顺着现场走一遍。
...
**探讨**：完整日志逐行过，报错落在 `/posts/11`——正是后台发的一篇纯数字标题的测试文章。根因在 YAML：序列化 frontmatter 时纯数字标题没加引号，gray-matter 按 YAML 规范解析成 `number`，渲染层 `.charAt()` 当场爆炸。**错误信息里的方法名是路标**：「charAt 不是函数」= 有个该是字符串的东西不是字符串。
...
**探讨**：前者纯 CSS：`pre::before` 一个伪元素，`box-shadow: 14px 0 0 #ffbd2e, 28px 0 0 #27c93f` 一次画出 mac 三灯，零 JS、不侵入 HTML 生成层。后者是「选题」问题而非「技术」问题——真正需要的不是外链，是**风格统一的图源**。最终用文生图 API 产出「纸上山水」八幅（与宣纸美学同宗），入库 `public/images/gallery/`，逐张验文件头 `FF D8 FF` 才收货。
...
第一版诊断走的是老路：`Symbol.asyncIterator` 是 ReadableStream 异步迭代协议的方法，老内核浏览器没有，于是往根布局 `<head>` 里塞了个 polyfill，自我感觉闭环。结果 polyfill 上线后报错原样重现，紧接着又撞上一个更要命的问题——**首页点文章跳不动了**。现代 Chrome 上必炸，老内核解释不通，第一版结论当场翻车。
...
探讨：业务代码没有任何 preload 调用。真凶是 React 19 的预取语义：路由预取 /gallery 的 RSC 载荷时，载荷里的图片被顺带 `preload` 进请求队列；访问者没有点进画廊，浏览器几秒后提醒「预加载没用上」。这是预取机制的副作用，无害。
